/*
  ...
  Security focus:
  - OAuth validation on MCP APIs only (Workshop MCP, Path MCP)
  - Content Safety on MCP APIs (Layer 1)
  - Path REST API has no auth (accessed via MCP layer)
...
// Policy for Workshop MCP API - OAuth + Content Safety (Layer 1 only)
...
// VS Code should use RFC 9728 path-based discovery: /.well-known/oauth-protected-resource/Workshop/mcp
...
// Path REST API Policy - passthrough (no auth, accessed via MCP layer)
...
// Policy for Path MCP API - OAuth + Content Safety (Layer 1 only)
...
// Note: Cannot add suffix-pattern PRM operation to MCP type API
// VS Code should use RFC 9728 path-based discovery: /.well-known/oauth-protected-resource/PATHS/mcp
