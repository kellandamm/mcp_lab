...
SERVER_NAME="${SERVER_NAME:-Workshop-mcp}"
API_PATH="${API_PATH:-Workshop/mcp}"
...
required_vars=(SUBSCRIPTION_ID RESOURCE_GROUP APIM_NAME Workshop_SERVER_URL)
...
az rest --method PUT \
  ...
  --body "$(jq -n \
    --arg url "${Workshop_SERVER_URL}/mcp" \
    '{
      properties: {
        protocol: "http",
        url: $url,
