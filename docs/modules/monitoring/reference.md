...
<div class="module-banner">
  <div class="module-banner-content">
    <div class="module-banner-text">
      <div class="module-banner-label">Module 4 · Queries & Logs</div>
      <h1>Reference</h1>
...
Module 4 uses four Azure Monitor components:
...
Module 4 has a **two-layer security architecture**. Both layers stream telemetry to the same Log Analytics workspace:
...
All four services (APIM, security function v1/v2, MCP server, Path API) report to a **single shared Application Insights** instance. This gives you a single pane of glass — KQL queries can join telemetry across services, and alerts span the entire system.
...
!!! note "Service Instrumentation"
    ...
    - **Path-api**: FastAPI instrumentation, appears in `AppRequests` when receiving HTTP traffic
    - **Workshop-mcp-server**: OpenTelemetry configured, appears in `AppTraces` (MCP uses **Streamable HTTP** transport, which supports both single JSON responses and SSE streaming for longer operations. APIM proxies these requests to the backend MCP server.)
...
| where TimeGenerated > ago(1h)
| where AppRoleName == "Workshop-mcp-server"
| where Message startswith "get_weather" or Message startswith "check_Path" or Message startswith "get_gear"
...
```kusto
// MCP tool invocations from Workshop-mcp-server
AppTraces
| where TimeGenerated > ago(24h)
| where AppRoleName == "Workshop-mcp-server"
| where Message startswith "get_weather" or Message startswith "check_Path" or Message startswith "get_gear"
| extend tool = case(
    ...
    Message startswith "check_Path", "check_Path_conditions",
...
```kusto
// MCP tool parameter analysis from Workshop-mcp-server
AppTraces
| where TimeGenerated > ago(24h)
| where AppRoleName == "Workshop-mcp-server"
| where Message startswith "get_weather" or Message startswith "check_Path" or Message startswith "get_gear"
| extend tool = case(
    ...
    "unknown")
| extend location = extract("location=([^\"\\)]+)", 1, Message),
         Path_id = extract("Path_id=([^\"\\)]+)", 1, Message),
         conditions = extract("conditions=([^\"\\)]+)", 1, Message)
| project TimeGenerated, tool, location, Path_id, conditions
| where isnotempty(location) or isnotempty(Path_id) or isnotempty(conditions)
```
...
| `PII_REDACTED` | Personal data found and masked in output | INFO | Normal operation, audit Path |
...
The Workshop-mcp-server returns **single JSON responses** for its simple tools. The connection closes after the complete response, so APIM can buffer and inspect the body.

```xml
<!-- This works because Workshop-mcp-server returns complete JSON responses -->
