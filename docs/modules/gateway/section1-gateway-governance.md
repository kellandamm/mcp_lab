<div class="module-banner">
  <div class="module-banner-content">
    <div class="module-banner-text">
      <div class="module-banner-label">Module 2 · MCP Gateway</div>
...
In this section, you'll deploy two MCP servers behind APIM: one native MCP server (Workshop) and one REST API exported as MCP (Path API). You'll configure OAuth with automatic discovery using Protected Resource Metadata (RFC 9728) and add rate limiting to protect your backends.

!!! tip "Working Directory"
    All commands in this section should be run from the `modules/gateway` directory:
    ```bash
    cd modules/gateway
    ```
...
| Workshop MCP Server | Receives clean, authenticated requests |
...
???+ note "Step 1: Deploy Vulnerable Server"
    ...
    Let's start by deploying the Workshop MCP Server with no authentication at all:
...
???+ danger "Step 2: Exploit - Anyone Can Access"
    ...
    **1. Get your endpoints:**
    ...
    === "Bash"
        ```bash
        azd env get-values | grep -E "Workshop_SERVER_URL|APIM_GATEWAY_URL"
        ```

    === "PowerShell"
        ```powershell
        azd env get-values | Select-String "Workshop_SERVER_URL|APIM_GATEWAY_URL"
        ```
    ...
    **2. Configure VS Code to connect:**
    ...
    ```json
    {
      "servers": {
        ...
        "Workshop-via-apim": {
          "type": "http", 
          "url": "https://your-apim-instance.azure-api.net/Workshop/mcp"
        }
      }
    }
    ```
    ...
    **3. Connect from VS Code:**
    ...
    - **Test 1: Direct Container App access**
        - Click the **Start** button above `Workshop-direct`
    ...
    - **Test 2: APIM Gateway access**  
        - Click the **Start** button above `Workshop-via-apim`
    ...
    **4. Invoke tools from either connection:**
    ...
    - `get_weather` - See current weather conditions
    - `check_Path_conditions` - View Path status
    ...
    ??? danger "Security Impact: Complete Exposure"
        **The vulnerability:** VS Code connected with zero authentication!
        ...
        :material-close: No audit Path of who accessed what
...
???+ success "Step 3: Fix - Add OAuth with PRM Discovery"
    ...
    **1. RFC 9728 PRM Metadata Endpoints**  
    ...
    - **RFC 9728 path-based:** `https://apim-xxxxx.azure-api.net/.well-known/oauth-protected-resource/Workshop/mcp`
    - **Suffix pattern:** `https://apim-xxxxx.azure-api.net/Workshop/mcp/.well-known/oauth-protected-resource`

    Both return the same PRM metadata:

    ```json
    {
      "resource": "https://apim-xxxxx.azure-api.net/Workshop/mcp",
            ...
            <return-response>
                ...
                <set-body>@{
                    return JsonConvert.SerializeObject(new {
                        resource = "{{apim-gateway-url}}/Workshop/mcp",
    ...
    **2. OAuth Validation Policy**  
    Applies token validation to the Workshop MCP API that:
    ...
    WWW-Authenticate: Bearer error="invalid_token", resource_metadata="https://apim-xxxxx.azure-api.net/Workshop/mcp/.well-known/oauth-protected-resource"
    ```
...
???+ note "Step 4: Validate - Confirm OAuth Works"
    ...
    **Expected output:**
    ...
    Test 2: Check WWW-Authenticate header has correct resource_metadata
      ✅ WWW-Authenticate includes /Workshop/mcp path

    Test 3: Check 401 response body has correct resource_metadata
      ✅ Response body includes /Workshop/mcp path

    Test 4: RFC 9728 path-based PRM discovery
      GET https://apim-xxxxx.azure-api.net/.well-known/oauth-protected-resource/Workshop/mcp
    ...
    Test 5: Suffix pattern PRM discovery
      GET https://apim-xxxxx.azure-api.net/Workshop/mcp/.well-known/oauth-protected-resource
    ...
    !!! tip "Test with VS Code"
        ...
        1. Restart the `Workshop-via-apim` connection from Step 2
...
!!! warning "Backend Still Exposed"
    OAuth is now enforced at the APIM gateway, but the Container App running Workshop is still publicly accessible. Anyone who discovers the direct Container App URL can bypass APIM entirely (as shown in Step 2's `Workshop-direct` test).
...
| Path REST API | REST | Receives standard HTTP requests |
...
???+ note "Step 1: Deploy Path API as MCP Server"

    Deploy the Path API and expose it as an MCP server through APIM:
    ...
    ??? info "What is the Path API?"
        **Path API** is a REST API that provides Path permit management:
        
        | Operation | Method | Path | Description |
        |-----------|--------|------|-------------|
        | `list_PATHS` | GET | `/PATHS` | List all available hiking PATHS |
        | `get_Path` | GET | `/PATHS/{id}` | Get details for a specific Path |
        | `check_conditions` | GET | `/PATHS/{id}/conditions` | Current Path conditions and hazards |
        | `get_permit` | GET | `/permits/{id}` | Retrieve a Path permit |
        | `request_permit` | POST | `/permits` | Request a new Path permit |
        ...
        **Example transformation:**
        ...
        /PATHS/{id}/conditions:
          get:
            ...
            summary: Get current Path conditions
            parameters:
              - name: id
                in: path
                required: true
        ...
        ```json
        {
          ...
          "description": "Get current Path conditions",
          "inputSchema": {
            "type": "object",
            "properties": {
              "id": { "type": "string" }
    ...
    - **Container App** running the Path API (REST API with OpenAPI spec)
    - **APIM backend** pointing to the Path API Container App
    ...
    **Expected output:**
    ...
    ```
    ==========================================
    Path API Deployed as MCP Server
    ==========================================
    
    Path Services Product:
      Subscription Key: a1b2c3d4...x9y0
    
    REST Endpoint: https://apim-xxxxx.azure-api.net/Pathapi/PATHS
    MCP Endpoint:  https://apim-xxxxx.azure-api.net/PATHS/mcp
    
    MCP Tools available:
      - list_PATHS: List all available hiking PATHS
      - get_Path: Get details for a specific Path
      - check_conditions: Current Path conditions and hazards
      - get_permit: Retrieve a Path permit
      - request_permit: Request a new Path permit
...
???+ danger "Step 2: Exploit - Subscription Keys Are Not Authentication"
    ...
    **1. Configure VS Code to connect:**
    ...
    Add the Path MCP server to `.vscode/mcp.json`:
    ...
    ```json
    {
      "servers": {
        "PATHS-via-apim": {
          "type": "http",
          "url": "https://your-apim-instance.azure-api.net/PATHS/mcp",
          "headers": {
    ...
    ```bash
    azd env get-value Path_SUBSCRIPTION_KEY
    ```

    **2. Connect and invoke tools:**

    - Click **Start** on `PATHS-via-apim`
    - Connection succeeds with the subscription key
    - Try invoking `list_PATHS` or `check_conditions`
    ...
    **3. The authentication problem:**
    ...
    ```bash
    # Alice uses the Path MCP server
    ...
    ??? danger "Understanding Subscription Keys vs Authentication"
        ...
        **Subscription keys are NOT good for:**
        ...
        - **Audit PATHS** - Logs show "engineering-key" not "bob@company.com"  
...
???+ success "Step 3: Fix - Add OAuth for Authentication (Keep Subscription Key for Tracking)"
    ...
    **Expected output:**
    ...
    ```
    ==========================================
    Waypoint 1.2: Add OAuth to Path MCP
    ==========================================
    ...
    ==========================================
    OAuth Added to Path MCP Server
    ==========================================

    PRM Discovery endpoint (RFC 9728):
      https://apim-xxxxx.azure-api.net/.well-known/oauth-protected-resource/PATHS/mcp
    ...
    ??? info "What This Script Deploys"
        ...
        **1. RFC 9728 PRM Metadata Endpoint**  
        Creates a discovery endpoint for the Path MCP server:
        
        - **RFC 9728 path-based:** `https://apim-xxxxx.azure-api.net/.well-known/oauth-protected-resource/PATHS/mcp`

        Returns PRM metadata:

        ```json
        {
          "resource": "https://apim-xxxxx.azure-api.net/PATHS/mcp",
          "authorization_servers": [
            "https://login.microsoftonline.com/your-tenant-id/v2.0"
          ],
          "scopes_supported": ["your-mcp-app-client-id/user_impersonate"],
        ...
        **Why only one endpoint?** In Waypoint 1.1, we created *two* PRM discovery endpoints for Workshop (RFC 9728 path-based and suffix pattern). Here we only create the RFC 9728 path-based endpoint because both patterns work and one is sufficient. VS Code's MCP client will try multiple discovery paths and use whichever responds. The suffix pattern (`/{path}/.well-known/oauth-protected-resource`) and RFC 9728 path-based pattern (`/.well-known/oauth-protected-resource/{path}`) both work. We demonstrated both in Waypoint 1.1 for educational purposes, but for Path MCP we keep it simple.

        **2. OAuth Validation Policy**  
        Adds token validation to the Path MCP API:
        ...
        ```
        HTTP/1.1 401 Unauthorized
        WWW-Authenticate: Bearer error="invalid_token", resource_metadata="https://apim-xxxxx.azure-api.net/PATHS/mcp/.well-known/oauth-protected-resource"
        ```
    ...
    ??? tip "Why Keep Both Subscription Keys AND OAuth?"
        ...
        **OAuth token provides:**
        ...
        - **Audit Path** - Log exactly who did what
...
???+ note "Step 4: Validate - Confirm Both Credentials Required"
    ...
    **Expected output:**
    ...
    ```
    ==========================================
    Waypoint 1.2: Validate Path MCP Security
    ==========================================
    ...
    Test 4: RFC 9728 PRM discovery
      GET https://apim-xxxxx.azure-api.net/.well-known/oauth-protected-resource/PATHS/mcp
    ...
    !!! tip "Test with VS Code"
        ...
        1. Keep subscription key in `.vscode/mcp.json`:
           ...
           ```
        2. Restart the `PATHS-via-apim` connection
        3. VS Code will discover OAuth via PRM and prompt you to sign in
        4. After authentication, invoke `list_PATHS` or `check_conditions`
...
Sending 20 rapid requests to Path API...
...
Apply rate limiting to the Path REST API:
...
??? info "What This Script Deploys"
    The script applies rate limiting to the Path REST API:
    
    | API | Path | Policy |
    |-----|------|--------|
    | **Path-api** | `/Pathapi/*` | Rate limiting by subscription key |
    
    **Why only Path API?** The Workshop MCP API uses OAuth tokens (from Waypoint 1.1), not subscription keys. Rate limiting by subscription key only makes sense for APIs that require subscriptions—which is why we added one in Waypoint 1.2!
    
    The Path API now enforces:
