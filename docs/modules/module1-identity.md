...
<div class="module-banner">
  <div class="module-banner-content">
    <div class="module-banner-text">
      <div class="module-banner-label">Module 1</div>
...
Welcome to **Module 1**, where you'll establish production-grade identity controls for your MCP server. In Module 0, you learned that unauthenticated servers are dangerous. Now we'll deploy to Azure and implement enterprise security using Managed Identity, Key Vault, and OAuth 2.1 with JWT validation.

This module demonstrates why the same vulnerabilities from Module 0 are even more dangerous in the cloud, and how Azure's identity services provide passwordless, production-grade solutions. You'll follow the same **"vulnerable → exploit → fix → validate"** methodology, but this time in a real cloud environment with real-world security controls.
...
!!! info "Module 1 at a Glance"
    ⏱️ **Time:** 45 min (demo) · 2 hours (full walkthrough)  
    💰 **Azure Cost:** ~$2–5/day while deployed (Container Apps + Key Vault + Container Registry)  
    🗑️ **Clean up when done:** `azd down --force --purge`  
    👉 **Recommended client:** [GitHub Copilot agent mode](../copilot-client.md)

## What You'll Learn

Building on Module 0's foundation, you'll master enterprise-grade identity and access management in Azure:
...
Before starting Module 1, ensure you have the required tools installed.
...
:material-check: Completed Module 0 (recommended)  
...
Module 1 follows six waypoints, each building on the previous one. Click each waypoint below to expand instructions.
...
??? note "Waypoint 1: Deploy Vulnerable Server to Azure"
    ...
    **What gets deployed:**
    ...
    - **Vulnerable server** — Uses static tokens (same pattern as Module 0, but now the risks are worse because tokens are visible in the Azure Portal)
    - **Secure server** — Pre-configured for OAuth 2.1 with JWT validation (you'll enable this in Waypoint 5)

    The vulnerable server uses the same `StaticTokenVerifier` pattern from Module 0, but now deployed to Azure where the vulnerabilities become even more dangerous.

    ??? info "What is StaticTokenVerifier? (Optional - Skip if you completed Module 0)"
        If you skipped Module 0, here's what you need to know:
        ...
        **Why this is insecure:**
        ...
        In this module, we'll migrate from this vulnerable pattern to **JWTVerifier** with OAuth 2.1, which solves all these problems using industry-standard authentication with Microsoft Entra ID.
    ...
    - **Environment name:** Choose a name (e.g., `module1-dev`)
...
??? danger "Waypoint 2: Exploit Cloud Vulnerabilities"
    ...
    The same vulnerabilities from Module 0 are more critical in Azure because:
    ...
    **Steps to exploit:**
    ...
    2. Navigate to your resource group (e.g., `rg-module1-dev`)
...
??? success "Waypoint 4: Migrate Secrets to Key Vault"
    ...
    **Expected output:**
    ...
    =======================================
    Loading azd environment...
    Creating demo secrets in Key Vault: kv-Workshop-module1-xxxxx
    ...
    1. Open [Azure Portal](https://portal.azure.com)
    2. Navigate to your Key Vault (e.g., `kv-Workshop-module1-xxxxx`)
    ...
    | **Audit** | No audit Path | Every access logged |
...
??? success "Waypoint 5: Upgrade to OAuth 2.1 with JWT Validation"
    ...
    ??? info "What is OAuth 2.1?"
        ...
        **In this module, you'll use two flows:**
    ...
    **Expected output:**
    ...
    ========================================
    Creating Entra ID app registration: Workshop-mcp-module1-1234567890
    ...
    ====================================
    App Name: Workshop-mcp-module1-1234567890
    ...
    **What's different in the code:**
    ...
    This module offers **two authentication methods**. Both demonstrate OAuth 2.1 security - choose based on your needs:
        ...
        **What happens:**
        ...
        ??? info "What's happening behind the scenes?"
            ...
            **Why this is more secure:**
            ...
            - Tokens are **tied to user identity** (audit Path)
    ...
    ??? example "Option B: Authorization Code + PKCE Demo (Production OAuth Flow)"
        ...
        **Step 1: Navigate to module1-identity**
...
??? note "Waypoint 6: Validate Security"
    ...
    **Expected output:**
    ...
    Name                        Identity
    --------------------------  -----------
    ca-Workshop-module1-xxxxx    UserAssigned
...
When you're done with Module 1, remove all Azure resources:
...
[Continue: Module 2: Gateway Security →](gateway/index.md){ .md-button .md-button--primary }

---

← [Module 0: Foundations](base-module.md) | [Module 2: Gateway Security](gateway/index.md) →
